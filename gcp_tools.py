...
class GoogleSheets:
    ...
    def check_credentials(self, sheet_id, scopes):
        ...
        parent_dir = os.path.dirname(os.path.dirname(
            os.path.abspath(__file__)))  # Get the parent directory path
        # Build the path to token.json in the parent directory
    ...
    def write_dataframe_to_sheet(self, dataframe, worksheet_name):
        ...
        try:
            ...
            if worksheet_name in existing_worksheets:
                ...
                worksheet.clear()
                self.logger.info(
                    f"Cleared existing worksheet '{worksheet_name}'.")
            else:
            ...
            self.logger.info(
                f"DataFrame written to {worksheet_name} in Google Sheets.")
        except Exception as e:
            self.logger.info(
                f"An error occurred while writing DataFrame to sheet: {str(e)}")


class GoogleCloudStorage:
    def __init__(self, credentials_info, logger=None, project=None):
        """
        ...
        """
        # 1. Set the environment variable if a path is provided
        if credentials_info:
