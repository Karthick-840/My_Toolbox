...
class ApiTools:
    ...
    def rapid_api_calls(self, rapid_api_dict, logger, params=None):
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    "Status code: {response.status_code}: Rapid API call Successfull")
            ...
            logger.info(
                f"Status code: {response.status_code}: An error occurred: {e}")
    ...
    def handle_status_code(self, response_status_code):
        ...
        if response_status_code in status_code_messages:
            self.logger.info(
                f"Status code: {response_status_code}: {status_code_messages[response_status_code]}")
        else:
            self.logger.info(
                f"Status code: {response_status_code}: Unexpected status code")
...
class Kaggle_Tools:
    ...
    def setup_kaggle_credentials(self):
        ...
        try:
            self.kaggle_json_path = os.path.join(
                self.kaggle_dir, 'kaggle.json')
            ...
            if os.path.exists(self.kaggle_json_path):
                ...
                self.logger.info(
                    "Json File Imported and loaded into environment")

                if self.move_to_read_only is not None:
                    self.kaggle_move_to_read_only()
                    self.logger.info(
                        f"Kaggle credentials loaded from {self.kaggle_json_path}")
                else:
                    self.logger.info(
                        f"kaggle.json not moved from {self.kaggle_dir}.")
    ...
    def kaggle_move_to_read_only(self):
        try:
            ...
            secure_kaggle_json_path = os.path.join(
                secure_kaggle_dir, 'kaggle.json')

            if platform.system() == "Windows":
                try:
                    os.rename(self.kaggle_json_path, secure_kaggle_json_path)
                    self.logger.info(
                        f'Moved Kaggle JSON to {secure_kaggle_json_path}')
                except Exception as e:
                    self.logger.info(
                        f'Error moving Kaggle JSON file in Windows: {e}')

                    # Verify that the file has been moved
                    if os.path.exists(secure_kaggle_json_path):
                        self.logger.info(
                            f'Success! The file has been moved to: {secure_kaggle_json_path}')

            else:
                # On Unix-like systems, use chmod
                try:
                    subprocess.run(
                        ['mv', self.kaggle_json_path, secure_kaggle_json_path])
                    subprocess.run(
                        ['chmod', '600', secure_kaggle_json_path], check=True)
                    self.logger.info(
                        f'Set permissions for {secure_kaggle_json_path} to 600.')
                except Exception as e:
                    self.logger.info(f'Error setting permissions in UNIX: {e}')

            self.logger.info(
                f"Moved kaggle.json to {secure_kaggle_json_path} and set it as readable only by the user.")
            ...
            self.logger.info(
                f"Error Moving or cleaning up Kaggle JSON file: {e}")
    ...
    def download_dataset(self, dataset_name: str):
        ...
        try:
            ...
            api.dataset_download_files(
                dataset_name, path=os.getcwd(), unzip=True)
...
class Git_Tools:
    ...
    def create_local_repo(self, repo_name, folder_path=None, remote_url=None):
        ...
        self.logger.info(
            f"Local repository '{repo_name}' created and initialized successfully.")
        ...
        if remote_url:
            ...
            self.logger.info(
                f"Repository '{repo_name}' linked to remote repository and changes pulled successfully.")
    ...
    def create_requirements(self, directory):
        try:
            self.logger.info(
                f"Generating requirements.txt for the directory: {directory}")
            # Use the full path to pipreqs in the virtual environment
            pipreqs_path = os.path.join(
                os.path.dirname(os.sys.executable), 'pipreqs.exe')
            print(pipreqs_path)
